...
export class GoogleAuthService implements IGoogleAuthService {
    public async handleGoogleAuthUser(payload: Profile, authUser: User | null): Promise<{
        status: string,
        message: string,
        user: Partial<User>;
        access_token: string;
    ...
    public async getUserByGoogleId(google_id: string): Promise<User | null> {
        try {
        return AppDataSource.getRepository(User).findOne({ where: { google_id }, relations: ["profile"] });
        } catch(error) {
            if (error instanceof HttpError) {
                throw error;
              }
