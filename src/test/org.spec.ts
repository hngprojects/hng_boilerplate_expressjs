...
describe('GET /api/v1/users/:id/organizations', () => {
  ...
  beforeAll(() => {
    token = jwt.sign({ userId }, '6789094837hfvg5hn54g8743ry894w4', { expiresIn: '1h' });
  ...
  it('should return 200 and the organizations for the user', async () => {
    const response = await request(app)
      ...
      expect(response.body.message).toBe('Invalid user ID or authentication mismatch.');
    });
  ...
  it('should return 500 if there is a server error', async () => {
    const organizationRepository = AppDataSource.getRepository(Organization);
    (organizationRepository.find as jest.Mock).mockRejectedValue(new Error('Database error'));
