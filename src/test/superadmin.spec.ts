...
describe("Update Organisation", () => {
  ...
  describe("updateUser", () => {
    it("should update the organosation successfully", async () => {
      ...
      expect(orgRepository.findOne).toHaveBeenCalledWith({ where: { id: "1" } });
    ...
    it("should throw a 404 error if user is not found", async () => {
      ...
      await expect(adminOrganisationService.update(req)).rejects.toThrow(HttpError);
      await expect(adminOrganisationService.update(req)).rejects.toThrow("Not Found");
      expect(orgRepository.findOne).toHaveBeenCalledWith({ where: { id: "2" } });
    });
